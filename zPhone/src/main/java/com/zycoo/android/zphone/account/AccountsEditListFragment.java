...
import com.zycoo.android.zphone.ui.contacts.CSSListFragment;
import com.zycoo.android.zphone.widget.DragnDropListView;
import com.zycoo.android.zphone.widget.DragnDropListView.DropListener;

import java.util.ArrayList;

public class AccountsEditListFragment extends CSSListFragment implements /*
                                                                          * OnQuitListener
                                                                          * ,
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            curCheckPosition = savedInstanceState.getLong(CURRENT_CHOICE, -1);
...
*/
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        lv.setOnDropListener(new DropListener() {
            ...
            public void drop(int from, int to) {
                ...
                // Finally add_to_contacts that in db
                //保存到数据库
                /**
                if (getActivity() != null) {
                    ContentResolver cr = getActivity().getContentResolver();
                    for (i = 0; i < orderedList.size(); i++) {
                        Uri uri = ContentUris.withAppendedId(SipProfile.ACCOUNT_ID_URI_BASE,
                                orderedList.get(i));
    ...
    private void showDetails(long profileId, String wizard) {
        ...
        /*
         * if (dualPane) { // If we are not currently showing a fragment for the
         * new // position, we need to create and install a new one.
         * AccountEditFragment df = AccountEditFragment.newInstance(profileId);
         * //df.setOnQuitListener(this); // Execute a transaction, replacing any
         * existing fragment // with this one inside the frame.
         * FragmentTransaction ft = getFragmentManager().beginTransaction();
         * ft.replace(R.id.details, df, null); //
         * ft.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE);
         * ft.setTransition(FragmentTransaction.TRANSIT_NONE); //if(profileId !=
         * Profile.INVALID_ID) { // ft.addToBackStack(null); //} ft.commit(); }
         * else {
         */
    ...
    /*
     * @Override public void onQuit() { curCheckPosition =
     * SipProfile.INVALID_ID; if(dualPane) { showDetails(curCheckPosition,
     * null); } }
     * @Override public void onShowProfile(long profileId) { curCheckPosition =
     * profileId; updateCheckedItem(); }
     */
   ...
   /* @Override
    public void onToggleRow(AccountRowTag tag) {
        ...
        getActivity().getContentResolver().update(
                ContentUris.withAppendedId(SipProfile.ACCOUNT_ID_URI_BASE, tag.accountId), cv,
                null, null);
    }
   ...
   /* @Override
     ...
     * @return corresponding sip profile if everything goes well, null if not
     *         able to retrieve profile
     */
    ...
    public boolean onContextItemSelected(android.view.MenuItem item) {
        ...
        switch (item.getItemId()) {
            case MENU_ITEM_DELETE: {
                getActivity().getContentResolver().remove_from_call_log(
            ...
            case MENU_ITEM_ACTIVATE: {
                ...
                getActivity().getContentResolver().update(
                        ContentUris.withAppendedId(SipProfile.ACCOUNT_ID_URI_BASE, account.id), cv,
                        null, null);
                return true;
            }
