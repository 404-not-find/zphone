...
public class NgnNetworkService extends NgnBaseService implements INgnNetworkService {
    ...
    //Add TQC
    //pptp
    private static final String PPTPVPN_INTERFACE_NAME = "ppp0";
    ...
    public String getLocalIP(boolean ipv6) {
        ...
        try {
            ...
            if (addressMap.size() > 0) {
                ...
                // Add TQC
                //pptpvpn address
                final InetAddress pptpvpn = addressMap.get(PPTPVPN_INTERFACE_NAME);
                if (pptpvpn != null) {
                    final String pptpvpnAddr = pptpvpn.getHostAddress().toString();
                    if (!NgnStringUtils.isNullOrEmpty(pptpvpnAddr)) {
                        return pptpvpnAddr;
                    }
                }
